...
sl2_frame_dtype = np.dtype(
    [
        ...
        (
            "id",
            "<u4",
        ),  # used to identify which sensor recordings belong together starting at 0 and counting up
        ...
        (
            "gps_speed",
            "<f4",
        ),  # Speed [knots]. Divide by 1.94385 to get m/s . Based on GPS NMEA.
        ...
        (
            "gps_altitude",
            "<f4",
        ),  # Altitude [feet]. Divide by 3.2808399 to get meters. Taken from GPS NMEA data.
...
class Sonar:
    ...
    def __init__(self, path, config, clean=True, augment_coords=False):
        self.path = path
        self.config = config
        ...
        self.frame_dtype = (
            sl3_frame_dtype if "sl3" in self.extension else sl2_frame_dtype
        )
    ...
    def _parse_header(self):
        self.version, self.device_id, self.blocksize, self.reserved = np.frombuffer(
            self.header, dtype="int16"
        )
    ...
    def _decode(self):
        ...
        self.df["frames"] = [
            np.frombuffer(
                self.buffer[(i + self.frame_header_size) : (i + p)], dtype="uint8"
            )
            for i, p in zip(self.df["first_byte"], self.df["frame_size"])
        ]

    def _bottom_index(self):
        frame_len = np.array([len(i) for i in self.df["frames"]])
        frame_bottom_index = (
            (frame_len / (self.df["max_range"] - self.df["min_range"]))
            * self.df["water_depth"]
        ).astype("int32")
    ...
    def _process(self):
        self.df[
            ["water_depth", "min_range", "max_range", "gps_altitude"]
        ] /= 3.2808399  # feet to meter
        self.df["gps_speed"] *= 0.5144  # knots to m/s
        self.df["survey"] = [
            self.channels.get(i, "unknown") for i in self.df["survey_type"]
        ]
        self.df["frequency"] = [
            self.frequencies.get(i, "unknown") for i in self.df["frequency_type"]
        ]
        self.df["seconds"] /= 1000  # milliseconds to seconds
        hardware_time_start = self.df["hardware_time"][0]
        self.df["datetime"] = pd.to_datetime(
            hardware_time_start + self.df["seconds"], unit="s"
        )
    ...
    def _valid_channels(self):
        ...
        self.valid_channels = [
            i for i in self.supported_channels if i in found_channels
        ]
    ...
    def water(self, channel: str, pixels: int) -> np.ndarray:
        ...
        frames_water_interp = [
            self._interp_water(f, d, pixels)
            for f, d in zip(frames_water, data["water_depth"])
        ]
    ...
    def bottom_intensity(self, channel: str) -> np.ndarray:
        ...
        data = self.df.query(f"survey == '{channel}'")
        bottom_intensity = np.array(
            [f[i] for f, i in zip(data["frames"], data["bottom_index"])]
        )
    ...
    def resize_proper_zoom_levels(self, df):
        ...
        # downsampling
        downsampled_rows = [
            downsample_row(row, ratio) for row, ratio in zip(image, max_range_ratio)
        ]
        ...
        image_resized = np.array(
            [
                np.pad(row, (0, max_length - len(row)), "constant")
    ...
    def points_shapefile(self, depth=True):
        ...
        geometry = [
            Point(lon, lat)
            for lon, lat in zip(sidescan_df["longitude"], sidescan_df["latitude"])
        ]
        gdf = gpd.GeoDataFrame(
            {"depth": sidescan_df["water_depth"]}, geometry=geometry, crs="EPSG:4326"
        )
        exporter = Exporter(self.config)
    ...
    def bathy_map(self):
        ...
        x = np.linspace(
            sidescan_df["longitude"].min(), sidescan_df["longitude"].max(), 1000
        )
        y = np.linspace(
            sidescan_df["latitude"].min(), sidescan_df["latitude"].max(), 1000
        )
        X, Y = np.meshgrid(x, y)
